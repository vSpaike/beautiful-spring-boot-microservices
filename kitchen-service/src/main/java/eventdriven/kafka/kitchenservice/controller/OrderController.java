...
public class OrderController {
    ...
    private static final List<Boolean> Workers = List.of(false, false, false, false, false);

    private static final Logger LOGGER = LoggerFactory.getLogger(OrderController.class);

    @PostMapping("/cook_order")
    public ServiceResponse addStock(@RequestBody Order order){
        LOGGER.info("Received order for cooking: {}", order);
        // Check if there is an available worker        int availableWorkerIndex = Workers.indexOf(false);
        if (Workers.contains(false)) {
            ...
            LOGGER.info("Order is being cooked by the worker for order: {}", order);
            return response;
        } else {
            ServiceResponse response = new ServiceResponse();
            response.setStatus(ResponseStatus.FAILURE);
            response.setMessage("No available workers to cook the order");
            LOGGER.warn("No available workers to cook the order for order: {}", order);
            return response;
        }
    }
